...
class FileAmountChecker:
    ...
    def folder_checker(self):
        htmlFolder = "Event_Record/Html/"
        logFolder = "Event_Record/Log/"
        try:
            if len(os.listdir(htmlFolder)) <= 10:
                ExceptionHandler(msg = "Html記錄檔無須進行刪除", exceptionLevel= "info")
            else:
                ExceptionHandler(msg = "Html記錄檔多於10個，進行刪除動作", exceptionLevel= "warning")
                while len(os.listdir(htmlFolder)) > 10:
                    os.remove(htmlFolder + os.listdir(htmlFolder)[0])
                ExceptionHandler(msg = "Html記錄檔已刪除剩至近10個", exceptionLevel= "info")
        except:
            ExceptionHandler(msg = "刪除Html記錄檔動作出現未知異常，需進行排除", exceptionLevel= "critical")
            raise
        try:
